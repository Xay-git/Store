...
Vue.component('smart-input', {
    template: `
        <div class="friendSearchContainer">
            ...
            <input v-else v-model="searchString" class="layui-input" style="padding-left: 10px;min-height: 38px;"
                :placeholder="placeholder"
                @click="init"
                @keyup="keyboardDown"
                @blur="blur"
            ...
            <ul v-show="searching&&searchString" class="friendSearchList">
                <li v-if="!filtered.length">暂无数据</li>
                <li v-else v-for="(item, index) in filtered"
                    :class="{'smartInput-active': selected.includes(item)}"
                    @click.stop="clickOne"
                    @mouseover="hoverOn"
                    :data-item="JSON.stringify(item)"
                >{{ item.name }}  {{item.phoneNumber?'('+item.phoneNumber+')':''}}
                </li>
            </ul>
    ...
    methods: {
        ...
        clickOne(e) {
            let target = e.target;
            clearTimeout(this.timer);
            var item = JSON.parse(target.dataset.item)
            this.$emit('data',item);
            this.searching = true;
            this.focusIndex = [].indexOf.call(target.parentElement.children, target);
            this.chooseOne(target,item);
        },
        // 删除一个选项
        deleteOne(e) {
